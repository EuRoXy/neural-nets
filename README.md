Neural networks via TensorFlow to predict:

- [insurance charge](tf.insurance.ipynb).
- [life expectancy](tf_expectancy.ipynb).

Neural networks via PyTorch to predict:

- [apartment rent](tor_rent.ipynb).
- [electric vehicle charging loads](pt_EVload.ipynb).
- [hotel cancellations](pt_cancel_hotel.ipynb).
- [tumor](pt_classi_img_tumor.ipynb) from microscopic **image classification**.
- [classify IMDB movie reviews](pt_IMDB_reviews.ipynb) with **BERT Transformers**.
- [following sentences in a book](pt_gen_text_pride.ipynb) (Pride and Prejudice) using **Character-Based LSTM**.